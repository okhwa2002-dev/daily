...
- 예외는 인증 **전** 이벤트를 기록하는 테이블뿐이다. `login_attempts`는 없는 계정으로 시도한 경우 행위자 ID가 존재하지 않으므로 `_by`를 두지 않고 `login_id`와 `ip`를 남긴다. 이런 예외를 새로 만들 때는 이 문서에 이유와 함께 적는다.
...
현재 `BOOK_GENRE`·`BODY_PART`·`INTENSITY` 셋이 이 방식이다. 나머지 코드 그룹은 기존대로 `codes.ts` + `CHECK` + zod enum을 쓴다.

**어느 쪽으로 갈지는 "값 집합이 바뀔 때 배포가 필요한가"로 가른다.** 장르·부위·강도는 라벨과 항목이 서비스 운영 중에 바뀔 수 있어야 하므로 공통코드다. `EXPENSE_KIND`(수입/지출)나 `BOOK_STATUS`처럼 코드가 값에 따라 분기하는 것들은 항목이 늘면 어차피 코드를 고쳐야 하므로 정적 enum이 맞다 — 그쪽을 공통코드로 옮기면 `CHECK`와 타입 강제를 잃으면서 얻는 것이 없다.

...
> **배포 주의:** `SCHEMA_VERSION` 4다. API를 먼저 배포하고 웹을 나중에 올린다. 반대 순서면 새 웹을 받은 사용자가 409 `SERVER_OUTDATED`를 받는다(큐는 보존되며 API 배포 후 자동 복구된다). 구버전 캐시 사용자는 426을 받고 새로고침 한 번으로 복구된다.

...
## 개정 — 부위·강도를 공통코드로 (2026-08-13)

이 문서 본문은 **구현 당시**의 설계다. 그 직후 부위·강도를 `codes` 테이블로 옮겼으므로 아래 세 곳은 더 이상 유효하지 않다. 본문을 고치지 않고 남겨 두는 것은 그때의 판단 근거를 지우지 않기 위해서다.

| 본문 | 현재 |
|---|---|
| §3 `bodyPart: z.enum(BODY_PART)` | `z.string().min(1)` — 값 집합이 `codes`에 있어 `z.enum`으로 막을 수 없다 |
| §4 "`validate`도 없다" | `validate`가 있다. 부위·강도를 `codes`와 대조하고, 모르면 `REJECTED`다 |
| §6 "라벨 맵을 `pages/workout/`에 둔다" | `labels.ts`는 삭제됐다. 라벨은 `codes` 캐시에서 `codeLabel`로 찾는다 |

바꾼 이유는 한글 라벨과 정렬 순서를 프론트 배포 없이 고치기 위해서다. 대가로 `BodyPart`·`Intensity` 타입과 `workouts_body_part_ck`·`workouts_intensity_ck`를 잃었다 — 그 방어는 서버의 sync 검증으로 옮겼다. `kind`는 코드가 값에 따라 분기하므로 정적 enum과 `CHECK`를 그대로 둔다.

`SCHEMA_VERSION`은 4 → 5다. 구버전 클라이언트는 관리자가 새로 넣은 코드를 다루지 못한다 — 라벨 자리가 비고, 수정 폼에 그 옵션이 없어 사용자가 다른 값을 고르는 순간 코드가 지워진다. 코드를 추가하는 시점에 올리는 것은 기억에 의존하므로, 값 집합을 여는 지금 올려 구버전을 먼저 걸러낸다.
